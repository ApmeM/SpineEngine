namespace SpineEngine.Maths
{
    ...
    public static class Mathf
    {
        ...
        public static float PingPong(float t, float length)
        {
            ...
            return length - (float)Math.Abs(t - length);
        }
        ...
        public static float Abs(float deltaAngle)
        {
            return (float)Math.Abs(deltaAngle);
        }
        ...
        public static Vector2 PointOnCircle(Vector2 circleCenter, float radius, float angleInDegrees)
        {
            ...
            return new Vector2
            {
                X = Cos(radians) * radius + circleCenter.X, Y = Sin(radians) * radius + circleCenter.Y
            };
        }
        ...
        public static Vector2 Lissajou(
            ...
            float phase = 0)
        {
            var x = Sin(elapsedSeconds * xFrequency + phase) * xMagnitude;
            var y = Cos(elapsedSeconds * yFrequency) * yMagnitude;
        ...
        {
            var wrappedTime = PingPong(elapsedSeconds, oscillationInterval);
            var damped = Pow(MathHelper.E, -damping * wrappedTime);

            var x = damped * Sin(elapsedSeconds * xFrequency + phase) * xMagnitude;
            var y = damped * Cos(elapsedSeconds * yFrequency) * yMagnitude;
